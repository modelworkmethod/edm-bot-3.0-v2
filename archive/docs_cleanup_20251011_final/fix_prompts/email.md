...
**Primary Flow:** Service-only feature → 1 service(s): EmailService

**Evidence**
- Commands: —
- Services: src\services\email\EmailService.js
- Tables: —
- Events: —
- Jobs: —
- Candidates: —
...
- - Path: `src/commands/email/email.js` (or similar)
- - Register in `src/commands/index.js`
- Add rate limiting entry in `src/middleware/RateLimiter.js`
- - Wire to existing service(s): src\services\email\EmailService.js
